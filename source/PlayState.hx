...
class PlayState extends MusicBeatState
{
	...
	public var daHealthGain:Float = 0.0;
	public var daHealthDeplete:Float = 0.0;
	public var healthDmg:Float = 0.0;

	public var isHealthGain:Bool = false;
	public var isHealthDeplete:Bool = false;
	...
	override public function create()
	{
		...
		rating.score = 200;
		rating.healthGain = 0.015;
		rating.healthDeplete = 0.0;
		rating.healthDmg = 0.015;
		rating.noteSplash = false;
		rating.gainHealth = true;
		rating.depleteHealth = false;
		ratingsData.push(rating);

		var rating:Rating = new Rating('bad');
		rating.ratingMod = 0.4;
		rating.score = 100;
		rating.healthGain = 0.0;
		rating.healthDeplete = 0.023;
		rating.healthDmg = 0.03;
		rating.noteSplash = false;
		rating.gainHealth = false;
		rating.depleteHealth = true;
		ratingsData.push(rating);
		...
		rating.score = 50;
		rating.healthGain = 0.0;
		rating.healthDeplete = 0.0475;
		rating.healthDmg = 0.045;
		rating.noteSplash = false;
		rating.gainHealth = false;
		rating.depleteHealth = true;
		ratingsData.push(rating);
		...
		if (!ClientPrefs.newHealthSystem && ClientPrefs.getGameplaySetting('stackableHealthDmg', true)) {
			ClientPrefs.gameplaySettings.set('stackableHealthDmg', false);
			trace('stackableHealthDmg requires newHealthSystem to activate the mechanic');
		}		
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			...
			if(WeekData.getWeekFileName() == 'mod2') { //for b-side
				switch (songName)
				{
					case 'spookeez-b' | 'south-b':
						curStage = 'spooky';
		...
		laneunderlaywhiteleftsideOpponent.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		laneunderlaywhiterightsideOpponent.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		laneunderlaywhiteleftside.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		laneunderlaywhiterightside.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		laneunderlaywhiteleftsidemiddlescroll.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		laneunderlaywhiterightsidemiddlescroll.alpha = ClientPrefs.laneUnderlayAlpha;
		...
		darkenScreen.alpha = ClientPrefs.darkenScreenAlpha;
		...
		var interpolateColor:Int->Int->Float->Int = function(startColor:Int, targetColor:Int, progress:Float):Int {
			var startRed:Int = (startColor >> 16) & 0xFF;
			var startGreen:Int = (startColor >> 8) & 0xFF;
			var startBlue:Int = startColor & 0xFF;
			
			var targetRed:Int = (targetColor >> 16) & 0xFF;
			var targetGreen:Int = (targetColor >> 8) & 0xFF;
			var targetBlue:Int = targetColor & 0xFF;
			
			var interpolatedRed:Int = startRed + Math.round((targetRed - startRed) * progress);
			var interpolatedGreen:Int = startGreen + Math.round((targetGreen - startGreen) * progress);
			var interpolatedBlue:Int = startBlue + Math.round((targetBlue - startBlue) * progress);
	...
	override public function update(elapsed:Float)
	{
		...
		if (healthBar.percent < 20) {
		    ...
		    if (!ClientPrefs.getGameplaySetting('stackableHealthDmg', false)) scoreTxt.color = 0xFFFF0000;
		}
		else
		{
			iconP1.animation.curAnim.curFrame = 0;
		    if (!ClientPrefs.getGameplaySetting('stackableHealthDmg', false)) scoreTxt.color = 0xFF00FF00;
		}
		...
		if (healthBar.percent >= 20 && healthBar.percent <= 80) {
			if (!ClientPrefs.getGameplaySetting('stackableHealthDmg', false)) scoreTxt.color = 0xFFFFFFFF;
		}

		var daTxtColor:Int;

		if (ClientPrefs.getGameplaySetting('stackableHealthDmg', false)) {
			if (healthDmg >= 0.4 && healthDmg <= 1.6) {
			...
			var startColor: Int = scoreTxt.color;
			var targetColor: Int = daTxtColor;
			var currentTime: Float = 0.0;
			var tweenDuration: Float = 0.5;

			scoreTxt.color = 0xFF00FF00; // starts with color green
			...
			if (currentTime >= tweenDuration) {
				...
				var progress: Float = currentTime / tweenDuration; // calculates the progress of the tween, simple math
				var interpolatedRed: Int = Math.round((startColor >> 16) + ((targetColor >> 16) - (startColor >> 16)) * progress); // calculates the interpolated value for the red color
				var interpolatedGreen: Int = Math.round(((startColor >> 8) & 0xFF) + (((targetColor >> 8) & 0xFF) - ((startColor >> 8) & 0xFF)) * progress); // calculates the interpolated value for the green color
				var interpolatedBlue: Int = Math.round((startColor & 0xFF) + ((targetColor & 0xFF) - (startColor & 0xFF)) * progress); // calculates the interpolated value for the blue color
				var interpolatedColor: Int = (interpolatedRed << 16) | (interpolatedGreen << 8) | interpolatedBlue; // combines the interpolated values of red, green, and blue color to create the final interpolated color
		
				scoreTxt.color = interpolatedColor; // sets the scoretxt into interpolatedColor
			}
		}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore && !ClientPrefs.getGameplaySetting('botplay', false))
			{
				...
				Highscore.saveScore(SONG.song, songScore, storyDifficulty, storyMode, percent);
				#end
			}
			playbackRate = 1;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		daHealthGain += daRating.healthGain * healthGain;
		daHealthDeplete += daRating.healthDeplete * healthLoss;
		healthDmg += daRating.healthDmg * healthLoss;

		isHealthGain = daRating.gainHealth;
		isHealthDeplete = daRating.depleteHealth;
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		var usedStackableHealthDmg:Bool = ClientPrefs.getGameplaySetting('stackableHealthDmg', false);

		if (usedStackableHealthDmg) {
			if (health <= 2) {
				health -= healthDmg;
		        healthDmg = 0;
			    daHealthGain = 0;
			}
			if (health <= 1.98 && health >= 0.02) {
			...
			if (ClientPrefs.newHealthSystem)
				{
					if (health > 0.023 * healthDrain)
					{
						if(!note.isSustainNote)
						{
							...
							if (health <= 0.023 * healthDrain) // health goes lower
							{
								health = 0.000000001;
							}
						}
				...
				if (ClientPrefs.newHealthSystem) {
					if (!note.isSustainNote) {
						if (!usedStackableHealthDmg) {
							...
							if (isHealthGain && health < 2) {
								health += daHealthGain;
								daHealthGain = 0;
							}
							if (isHealthGain && health <= 1.98 && health >= 0.02) {
								healthDmg = 0;
							}
							if (isHealthDeplete && health <= 1.98 && health >= 0.02) {
								health -= daHealthDeplete;
								daHealthDeplete = 0;
								healthDmg = 0;
							}
